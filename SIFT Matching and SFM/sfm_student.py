...
class SFMSolver(object):
    ...
    def __init__(self, img_pattern, intrinsic, output_dir, t, downscale=1):
        """
    ...
    def visualize_matches(self, img1, img2,
        ...
        if mask is not None:
            if not isinstance(mask, list):
                ...
                matchesMask = mask
            #matchesMask=[1 if 0 else 0 for i in matchesMask]
    ...
    def triangulate(self, p1, p2, R, trans, mask):
        ...
        rotation=np.pad(R, [[0,0], (0, 1)], mode='constant')
        ...
        print("normalised",p2_n)

        #Apply triangulation
        pts4D = cv2.triangulatePoints(rotation, rotationFirst, p2_n, p1_n).T
    ...
    def run(self):
        ...
        p1, p2, matches_good, kp1, kp2, matches_not_good = self.detect_and_match_feature(
            self.imgs[img1], self.imgs[img2])

        self.visualize_matches(
            self.imgs[img1], self.imgs[img2], kp1, kp2, matches_good, save_path=join(self.output_dir, 'sift_match.png'))
        ...
        self.visualize_matches(
            self.imgs[img1], self.imgs[img2], kp1, kp2, matches_good, mask=mask,
            save_path=join(self.output_dir, 'inlier_match.png'))

        negate_mask= [1 if i==0 else 0 for i in mask]

        self.visualize_matches(
            self.imgs[img1], self.imgs[img2], kp1, kp2, matches_good, mask=negate_mask,
            save_path=join(self.output_dir, 'oulier_match.png'))


        self.visualize_epipolar_lines(
            self.imgs[img1], self.imgs[img2], p1, p2, E,
        ...
        self.write_simple_obj(point_3d, None, filepath=join(
            self.output_dir, 'output'+str(img1)+str(img2)+str(self.t)+".obj"))
...
def main():
    ...
    img_pattern = '/Users/nishatiwari/PycharmProjects/Hw3/hw3_code_data/cs677_hw3_data/rdimage.???.ppm'
    intrinsic = intrinsic_reader('/Users/nishatiwari/PycharmProjects/Hw3/hw3_code_data/cs677_hw3_data/intrinsics.txt')
    output_dir = './output'
    safe_mkdir(output_dir)
    T = [.7]
    for i in T:
        print(i)
        sfm_solver = SFMSolver(img_pattern, intrinsic, output_dir, i,downscale=2)
        sfm_solver.run()
